...
class LikeProfile extends StatefulWidget {
   ...
   LikeProfile({Key? key }) : super(key: key);
...
class _LikeProfileState extends State<LikeProfile> {
    ...
    final UserLikesController userController = Get.put(UserLikesController());
    final userData2 = GetStorage();
    Future<List<UserLike>?> getData2() async {
      final response = await http.get(Uri.parse(
          'https://katkoty-app.com/admin/api/community/get_user_likes.php?user_id=${userData2.read('id')}'));


      if (response.statusCode == 200) {
        var data = jsonDecode(response.body.toString());
        print(data);
        List jsonData = (data['user_like']);
        return jsonData.map((Data) => UserLike.fromJson(Data)).toList();
      } else {
    ...
    return SizedBox(
      ...
      child: FutureBuilder<List<UserLike>?>(
        ...
        builder: (context,snapshot){
          ...
          if(snapshot.hasData){
            List<UserLike>? item = snapshot.data;

            return item!.length==0? Text('لا يوجد اعجابات',style: TextStyle(
              color: Colors.black,
              fontSize: 20,
            ),):ListView.separated(
